...
手动添加公众号或转载网页时，系统会读取页面元数据及正文中的“来源 / 转载自 / 据……报道”等署名，并在同一次 LLM 成稿调用中补充核验原始发布方。`公众号 / 网站` 只作为载体记录，不再写入 Word 的来源字段。如果转载页包含发布方原文链接，候选卡片和 Word 来源会链接到该原文；未包含时链接到原始媒体首页，同时保留转载页面供人工复核。
